...
const findIdsWithoutDataDependencies = () => {
  ...
  const notTrackedIds = _.difference([...Array.from(state.pages.values(), p => p.matchPath?p.matchPath:p.path), ...[...state.staticQueryComponents.values()].map(c => c.jsonName)], [...allTrackedIds, ...seenIdsWithoutDataDependencies]); // Add new IDs to our seen array so we don't keep trying to run queries for them.
...
const runQueriesForPathnames = pathnames => {
  ...
  pageQueries.forEach(id => {
    ...
    if (page) {
      ...
      queue.push({
        id: page.matchPath?page.matchPath:page.path,
